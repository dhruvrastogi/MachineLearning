...
This is a course project worked towards the 'Practical Machine Learning' MOOC on Coursera. The course has been organized by Dr. Jeff Leek from the John Hopkins Bloomberg School of Health. 
...
M <- abs(cor(training[,-ncol(training)]))>0.8
diag(M) <- 0
corColumns <- which(M>0.8,arr.ind=T)
training <- training[,-seq(1,nrow(corColumns),2)]
testing <- testing[,-seq(1,nrow(corColumns),2)]
```
At the end of the feature selection step and before the model building step, the training data contains a total of 35 covariates
...
qplot(magnet_dumbbell_y, roll_forearm,colour=classe,data=training)
```
...
```
The out of sample accuracy is the best for A (90%) and is expected to be nearly 50% averaged across all the five classes
